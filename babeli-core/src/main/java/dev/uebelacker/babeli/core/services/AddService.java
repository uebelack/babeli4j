...
public class AddService {
  ...
  public void add(String key, Map<String, String> translationValues) {
    ...
    if (configuration.getFile() != null) {
      ...
      fileWriter.writeFile(
          new MultiLanguageTranslationFile(translationFile.file(), translations.getTranslations()));
    }
  ...
  private String resolveValue(
    ...
    // Find a source language+value to translate from: prefer base language, then any other provided
    // value
    var baseLanguage = configuration.getBaseLanguage();
    var sourceLanguage =
        translationValues.containsKey(baseLanguage) && translationValues.get(baseLanguage) != null
            ? baseLanguage
    ...
    if (sourceLanguage == null) {
      log.warn(
          "No source translation available to derive value for language '%s' and key '%s'. Skipping."
              .formatted(language, key));
      return null;
    }

    var sourceValue = translationValues.get(sourceLanguage);
    log.info(
        "Generating translation for key '%s' and language '%s' from '%s'."
            .formatted(key, language, sourceLanguage));
    var translation = translationService.translate(sourceValue, sourceLanguage, language);
    log.info(
        "Generated translation for key '%s' and language '%s': '%s'"
            .formatted(key, language, translation));
    return translation;
  }
}
